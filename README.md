...
**July 2026 -- code review pass 2 (importRdata SVA crash, DRIMSeq feature-level bug, DRY consolidation):**

* **Fixed:** `run_isoform_switch()` could crash deep inside
  `IsoformSwitchAnalyzeR::importRdata()` with `Error in limma::makeContrasts():
  The levels must be syntactically valid names` (e.g. names like
  `sv1-0.127953077968942`). `importRdata()` runs its own internal SVA
  (surrogate-variable) fit as part of `estimateDifferentialGeneRange` and
  `detectAndCorrectUnwantedEffects`, and a discovered surrogate variable's
  numeric value can end up baked into a design-matrix column name, which
  isn't syntactically valid R and crashes `limma::makeContrasts()`. Since
  ExpressOM's own design matrix here is always just `sampleID`+`condition`
  (no batch/cofactor column is exposed to `run_isoform_switch()` yet),
  `detectAndCorrectUnwantedEffects` would only ever be "correcting" for a
  covariate discovered by that fragile automatic path, and
  `estimateDifferentialGeneRange` is, by IsoformSwitchAnalyzeR's own
  documentation, just a rough pilot estimate that "cannot be trusted" and
  that ExpressOM never reads back out anyway. Both are now switched off
  (the latter only if the installed IsoformSwitchAnalyzeR version actually
  exposes the argument), which sidesteps the crash and also speeds up import
  by skipping two computations whose results were never used.
* **Fixed (significant):** `run_dtu()` called `DRIMSeq::results(d)` with no
  `level` argument, which returns DRIMSeq's *gene*-level results (columns:
  `gene_id`, `lr`, `df`, `pvalue`, `adj_pvalue` -- no `feature_id`) instead of
  transcript-level results. This silently discarded the transcript-level
  detail that is the entire point of a *Differential Transcript Usage* test,
  and meant `run_isoform_switch(..., test_engine = "DRIMSeq")` could never
  succeed: that path's own `required_cols` check demands a `feature_id`
  column that gene-level results never have. Fixed by requesting
  `DRIMSeq::results(d, level = "feature")`. `dtu_results$dtu_results` is now
  one row per transcript (with `gene_id` alongside), and the DRIMSeq
  `test_engine` option is now actually usable.
* **Fixed:** `plotP_fork()` (the SPIA two-way evidence plot) could pass a
  non-finite value into `geom_path()`/`geom_line()` when the FDR/FWER
  threshold couldn't be computed cleanly (e.g. a very small `nrow(na.omit(x))`
  pushing the derived threshold to a value `qchisq()`/`qnorm()` can't invert),
  producing a silent `Removed N rows containing missing values` warning and a
  truncated threshold line. Both threshold-line branches (Fisher and
  norm-inverse) now check finiteness before adding the layer, and log a
  message instead if a threshold line can't be drawn.
* **Mitigated:** IsoformSwitchAnalyzeR's own internal code (not ExpressOM's --
  there is no `dplyr::across()` call anywhere in this package) uses the
  deprecated `dplyr::filter(dplyr::across())` pattern without supplying
  `.cols`, which raises one lifecycle-deprecation warning *per row* on a
  genome-wide isoform/exon annotation -- hundreds of thousands of warnings
  from a single `importRdata()`/`analyzeAlternativeSplicing()` call in
  practice. A new `.muffle_across_deprecation()` helper selectively muffles
  only that one message pattern around those two calls; every other warning
  still surfaces normally.
* **DRY fix (significant):** `run_dtu()` (DRIMSeq engine) and
  `run_dexseq_dtu()` (DEXSeq engine) each independently implemented the same
  ~60-line sequence of sample-restriction, gene-mapping, and
  expression-based filters. Consolidated into a shared
  `.prepare_dtu_counts()` helper; the two genuinely engine-specific filters
  (DEXSeq's manual gene-level expression filter, since DRIMSeq's own
  `dmFilter()` already does the equivalent internally, and DEXSeq's ">= 2
  features per group" requirement) are preserved as opt-in arguments so each
  engine's exact prior filtering behaviour is unchanged.
* **DRY fix:** `import_counts()` (mod_dge.R) and `import_transcript_counts()`
  (mod_isoform.R) each had their own copy of the `remove_sample`/
  `subset_sample` filtering logic, and the copies had already drifted: the
  isoform copy silently dropped both the progress messages and the
  `tryCatch()` around `subset_sample`, so a typo'd `subset_sample` expression
  there raised a raw, unhelpful `eval()` error instead of the friendlier
  message the DGE path gives. Consolidated into `.apply_sample_filters()`;
  both callers (and both error messages) are now consistent, and a
  `subset_sample` that matches zero samples is now caught immediately with a
  clear error instead of failing confusingly further downstream.
* **DRY fix:** the per-row z-score computation (`(x - row_mean) / row_sd`,
  guarding against divide-by-zero for a flat row) was hand-copied identically
  in `plot_sample_zscore()`, `plot_geneset_zscore_avg()`, and
  `plot_gene_zscore_individual()` (mod_plots.R). Consolidated into
  `.zscore_matrix()`.
* **Added:** a `DTU_top_barplot.pdf` (top significant DTU transcripts, ranked
  by adjusted p-value) in `generate_dte_dtu_report()`, alongside the existing
  DTU p-value histogram -- the DTU section previously had one figure to DTE's
  three. Now included in `dte_dtu_report.Rmd`. (DRIMSeq's feature-level test
  statistic is an unsigned likelihood-ratio stat, not a fold-change, so this
  ranks by `-log10(adj. p-value)` rather than mirroring DTE's fold-change bar
  height.)
* **Verified, no change needed:** re-checked `run_cpat.py` / CPAT detection
  end-to-end (`debug_wsl()` in utils_isoform.R and
  `.run_external_predictors()` in mod_isoform.R) against the fix described in
  the entry below -- both correctly treat `cpat`/`run_cpat.py` (and
  `signalp6`/`signalp`) as alternatives where either being present is
  sufficient, so this is confirmed still working correctly.
...
* **Fixed:** `run_isoform_switch()`'s `importRdata()` call always passed
  ...
  and this README, which claimed they were "always on". They're intentionally
  kept `FALSE`: ExpressOM now does its own selective, per-ID normalization of
  the FASTA headers and GTF `transcript_id` values before `importRdata()` ever
  sees them (the same logic as `clean_id()`), which makes `importRdata()`'s
  own cruder truncation both redundant and, in some cases, actively harmful
  (e.g. collapsing distinct IDs like `PB.1.1`/`PB.1.2` down to the same
  `PB.1`). Genuinely mismatched references are now caught earlier by a
  pre-check -- see the SQANTI3 guide above. (This entry previously described
  the opposite fix -- turning all three tolerance flags *on* -- which was the
  right call at the time but has since been superseded by this more precise
  approach; the changelog text just hadn't caught up.)
