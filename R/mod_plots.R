...
#' Transcript-level PCA plot
#'
#' Performs PCA on log2(counts + 1) of the transcript‑level count matrix,
#' using the same styling as the gene‑level PCA. Optionally applies limma
#' batch correction if a batch column is provided.
#'
#' @param isoform_obj List returned by `import_transcript_counts()`
#' @param condition_col Character column name in metadata for colour grouping
#' @param level Foreground condition label (used for file naming and optional subsetting)
#' @param base Reference condition label (used for file naming and optional subsetting)
#' @param out_dir Output directory (plots saved in `Plots/` subfolder)
#' @param batch_col Optional batch column for correction and shape
#' @param subset_to_contrast Logical; if TRUE, only samples with condition %in% c(base, level) are used.
#'   Default FALSE (use all samples). When FALSE, level and base are only used for plot titles/file names.
#' @param ... Additional arguments passed to `plot_custom_pca()` (e.g., `ellipse`)
#' @return Invisibly, the PCA plot object(s) – two if batch correction is used.
#' @export
plot_transcript_pca <- function(isoform_obj, condition_col, level, base,
                                out_dir, batch_col = NULL,
                                subset_to_contrast = FALSE, ...) {
  if (!requireNamespace("SummarizedExperiment", quietly = TRUE))
    stop("SummarizedExperiment is required for transcript PCA.")
  if (!requireNamespace("limma", quietly = TRUE) && !is.null(batch_col))
    warning("limma not installed; batch correction will be skipped.")
  ...
  # Optionally subset to the two conditions of interest
  if (isTRUE(subset_to_contrast)) {
    keep <- meta[[condition_col]] %in% c(base, level)
    if (sum(keep) < 2) {
      warning("Fewer than 2 samples in the comparison groups; PCA may be meaningless.")
    }
    counts <- counts[, keep, drop = FALSE]
    meta   <- meta[keep, , drop = FALSE]
    # Relevel condition factor so that base is the reference (for consistent colour ordering)
    meta[[condition_col]] <- factor(meta[[condition_col]], levels = c(base, level))
  } else {
    # Ensure condition is a factor (if not already)
    if (!is.factor(meta[[condition_col]])) {
      meta[[condition_col]] <- as.factor(meta[[condition_col]])
    }
    # Keep all samples; no releveling
  }
  ...
  se <- SummarizedExperiment::SummarizedExperiment(
    assays = list(logcounts = log_counts),
    colData = meta
  )
  ...
  # ---- Original (uncorrected) PCA ----
  p_orig <- plot_custom_pca(se, condition = condition_col, batch = batch_col,
                            title = paste("Transcript-level PCA (log2+1) -", comp_name),
                            return_plot = TRUE, ...)
  pdf(file.path(plot_dir, paste0("TranscriptPCA_", comp_name, ".pdf")),
      width = 9, height = 7)
  print(p_orig)
  dev.off()
  message("   -> Transcript-level PCA (uncorrected) saved to: ", plot_dir)

  # ---- Batch-corrected PCA (if batch_col provided) ----
  if (!is.null(batch_col) && batch_col %in% colnames(meta) &&
      requireNamespace("limma", quietly = TRUE)) {
    batch_vec <- meta[[batch_col]]
    if (length(unique(batch_vec)) > 1) {
      # Design matrix: if subset_to_contrast is TRUE, we use the subsetted factor;
      # otherwise, use the full factor with all levels.
      design_mat <- model.matrix(as.formula(paste0("~ ", condition_col)), data = meta)
      corrected_mat <- tryCatch(
        limma::removeBatchEffect(log_counts, batch = batch_vec, design = design_mat),
        error = function(e) { warning("Batch correction failed: ", e$message); NULL }
      )
      if (!is.null(corrected_mat)) {
        se_corr <- se
        SummarizedExperiment::assay(se_corr, "logcounts") <- corrected_mat
        p_corr <- plot_custom_pca(se_corr, condition = condition_col, batch = batch_col,
                                  title = paste("Transcript-level PCA (batch-corrected) -", comp_name),
                                  return_plot = TRUE, ...)
        pdf(file.path(plot_dir, paste0("TranscriptPCA_BatchCorrected_", comp_name, ".pdf")),
            width = 9, height = 7)
        print(p_corr)
        dev.off()
        message("   -> Transcript-level PCA (batch-corrected) saved to: ", plot_dir)
        return(invisible(list(original = p_orig, corrected = p_corr)))
      }
    }
  }
  invisible(p_orig)
