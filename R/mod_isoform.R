...
.run_external_predictors <- function(switch_list,
  ...
  if (is.null(sp_fa_w)) {
    ...
    if (has_sp6) {
      ...
      sp_cmd <- sprintf(
        ...
        shQuote(sp_fa_w, type = "sh"),
        shQuote(sp_prefix_w, type = "sh")
      )

      message("  Running SignalP 5...")
      sp_status <- run_tool(sp_cmd, tool_name = "SignalP5")
  ...
  .log_predictor_status(
    "SUMMARY",
    if (n_domain > 0 || n_sigp > 0 || n_cds > 0) "OK" else "EMPTY",
    summary_msg
  )
...
generate_dte_dtu_report <- function(dte_results,
  ...
  if (nrow(dte_filtered) > 0) {
    ...
    ggplot2::ggsave(
      file.path(plot_dir, "DTE_top_barplot.pdf"),
      p_bar,
      width = 10,
  ...
  if (nrow(dtu_sig) > 0) {
    ...
    ggplot2::ggsave(
      file.path(plot_dir, "DTU_top_barplot.pdf"),
      p_dtu_bar,
      width = 10,
      height = max(6, top_n * 0.3)
    )
  ...
  safe_run(
    {
      ...
      if (length(sig_sets) >= 2) {
        ...
        ggplot2::ggsave(
          file.path(plot_dir, "Engine_concordance.pdf"),
          p_concordance,
  ...
  if (!is.null(genes_of_interest)) {
    ...
    for (gene_sym in genes_of_interest) {
      ...
      ggplot2::ggsave(
        file.path(plot_dir, paste0("proportions_", gene_sym, ".pdf")),
        p_prop,
        width = 10,
        height = 6
      )
  ...
  if (has_switch) {
    ...
    if (all(c("dIF", "isoform_switch_q_value") %in% colnames(feat_sw))) {
      ...
      if (nrow(feat_sw_plot) > 0) {
        ...
        ggplot2::ggsave(
          file.path(plot_dir, "IsoformSwitch_overview.pdf"),
          p_switch_overview,
          width = 7,
          height = 6
        )
  ...
  if (n_unconverted > 0) {
    ...
    writeLines(
      c(
        ...
        "report.html embeds figures as PNG because browsers cannot display PDF via <img>.",
